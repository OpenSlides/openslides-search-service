...
CATCH=0
DOCKER_EXEC="docker exec search-test"
...
then
    # Container Mode
    if [ -z "$SKIP_CONTAINER_UP" ]; then docker run -d -t --name search-test ${IMAGE_TAG} || CATCH=1; fi
...
if [ -z "$PERSIST_CONTAINERS" ] && [ -z "$SKIP_CONTAINER_UP" ]; then docker stop search-test && docker rm search-test || CATCH=1; fi

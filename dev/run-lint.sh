...
# Safe Exit
trap 'if [ -z "$PERSIST_CONTAINERS" ] && [ -z "$SKIP_CONTAINER_UP" ]; then docker stop search-test && docker rm search-test; fi' EXIT
...
then
    # Container Mode
    if [ -z "$SKIP_CONTAINER_UP" ]; then docker run -d -t --name search-test ${IMAGE_TAG}; fi
